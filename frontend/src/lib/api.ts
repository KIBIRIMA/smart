...
export async function apiFetch<T = any>(path: string, init: RequestInit = {}, retry = true): Promise<T> {
  ...
  // Timeout long pour les appels lents (optimisation OR-Tools ~45s).
  // Les autres appels gardent un timeout raisonnable.
  ...
  try {
    ...
    if (e?.name === "AbortError") {
      throw new Error("Le calcul prend plus de temps que prévu. Réessayez : le cache est maintenant chaud, ce sera plus rapide.");
    }
    throw e;
  }
  clearTimeout(timer);
