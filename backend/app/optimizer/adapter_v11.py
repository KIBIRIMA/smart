...
def run_optimization_v11(missions, params=None) -> dict:
    ...
    def _osrm_route(a, b, rc=None):
        ...
        # Cache : évite de redemander une route déjà calculée (crucial en matrice
        # 40×40 = 1600+ routes). La clé reprend le format du moteur.
        key = None
        if rc is not None:
            key = f"{a['lat']:.5f},{a['lng']:.5f}|{b['lat']:.5f},{b['lng']:.5f}"
            if key in rc:
                return rc[key]
        res = None
        try:
    ...
    def _geocode_gouv(address, cache):
        ...
        try:
            r = _rq.get("https://api-adresse.data.gouv.fr/search/",
                        params={"q": address, "limit": 1}, timeout=5)
            j = r.json()
            ...
            if j.get("code") == "Ok":
                ...
                for i, pa in enumerate(pts):
                    for k, pb in enumerate(pts):
                        ...
                        if d is None:
                            continue
                        key = f"{pa['lat']:.5f},{pa['lng']:.5f}|{pb['lat']:.5f},{pb['lng']:.5f}"
