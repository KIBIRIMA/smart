...
def _analyser_pdf_vgp(chemin: Path, parc_attendu: str) -> dict:
    ...
    # Parc — plusieurs stratégies parce que les tableaux PDF sont extraits
    # avec des cellules éclatées. Ordre : label direct → "Code" suivi d'un
    # nombre puis "entreprise" (format CADET) → défaut.
    parc_detecte = None
    m_parc = _re.search(
    ...
    # 3bis. Modèle machine — chercher d'abord "Type : XXX" (rapports CADET/Apave),
    # puis "Marque : XXX Type : YYY". En dernier recours seulement, "Machine : …".
    # Rejeter tout ce qui ressemble à du texte descriptif ou à un nom d'entreprise.
    modele_detecte = None
    for pattern in (
        ...
        if m:
            ...
            # rejet des faux positifs (mots-clés qui ne sont PAS un modèle)
            ...
            if any(x in cand.lower() for x in interdits):
                continue
            # exiger au moins un chiffre ou une majuscule interne (modèles réels type DX30D, HA20RTJ)
    ...
    observations = None
    m_obs = _re.search(
        r"(?:observations?|remarques?|r[ée]serves?|anomalies?)\s*[:\-]?\s*\n+"
        r"(.{5,600}?)(?:\n\s*\n|Rapport transmis|Accr[ée]ditation|$)",
        t, _re.I | _re.S)
    if m_obs:
        observations = _re.sub(r"\s+", " ", m_obs.group(1)).strip()[:500]
        # rejeter les blocs vides ou qui commencent par du texte structurel
        if not observations or _re.match(r"^(rapport|contribuons|liste|acc[ré])", observations, _re.I):
            observations = None
