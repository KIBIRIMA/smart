...
def _analyser_pdf_vgp(chemin: Path, parc_attendu: str) -> dict:
    ...
    # 4. Modèle — "Constructeur : X … Type : Y" (même ligne possible), puis
    # "Type :" seul, borné aux libellés suivants (Heures, N°, Énergie…).
    ...
    if anomalie is None:
        if _re.search(
            ...
            r"(?:la\s+(?:re)?mise|de\s+mettre)\s+(?:à\s+(?:la\s+)?disposition|l'appareil)", tl):
            anomalie = "OUI"
        elif _re.search(
